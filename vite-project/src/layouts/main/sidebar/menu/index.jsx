        ...
        <div className="mt-0.5" key={account}>
            {
                maninMenu.map((menu, index) => (
                    <NavLink to={typeof menu.path === "function" ? menu.path() : menu.path} className='py-1 group' key={index}>
                        {({ isActive }) => (
                            <div className={classNames("inline-flex items-center  justify-start gap-5 px-2 group-hover:bg-[color:var(--background-secondary)] rounded-full py-3 relative ", { "font-bold": isActive })}>
